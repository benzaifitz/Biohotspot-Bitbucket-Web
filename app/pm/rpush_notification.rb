ActiveAdmin.register Rpush::Client::ActiveRecord::Notification, as: 'Notification', namespace: :pm do

  menu label: 'Notifications List', parent: 'Notifications'

  # config.clear_action_items!

  # action_item :view, only: :index do
  #   link_to 'Email Notification', "#{new_pm_notification_path}?notification_type=#{RpushNotification::NOTIFICATION_TYPE[:email]}"
  # end
  # action_item :view, only: :index do
  #   link_to 'Push Notification',  "#{new_pm_notification_path}?notification_type=#{RpushNotification::NOTIFICATION_TYPE[:push]}"
  # end
  ...
  index do
    ...
    column 'Recipient' do |notification|
      link_to(notification.user.username, pm_user_path(notification.user)) rescue nil
    end
    ...
    column 'Actions' do |nt|
      link_to 'View', pm_notification_path(nt), class: 'fancybox member-link', data: { 'fancybox-type' => 'ajax' }
    end
  end
  ...
  controller do
    ...
    def create
      ...
      if attrs[:alert].present?
        ...
        redirect_to "#{new_pm_notification_path(@rpush_notification)}?notification_type=#{attrs[:notification_type]}"
      end
    end
  end
end
