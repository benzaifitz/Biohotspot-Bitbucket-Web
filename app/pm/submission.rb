ActiveAdmin.register Submission, namespace: :pm do
  ...
  #   link_to 'Manual Entry', new_pm_submission_path
  # end
  index do
    selectable_column
    id_column
    column 'Sample' do |s|
      ...
      link_to(sc.name, pm_sample_path(sc.id)) if sc
    end
    column 'Species' do |s|
      link_to(s.category.name, pm_species_path(s.category.id)) rescue nil
    end
    column :site
    column :location
    column :project
  ...
  form do |f|
    ...
    f.inputs do
        ...
        f.has_many :sample_image, heading: 'Sample Photo*', new_record: false do |s|
          s.input :file, :as => :file, :hint => s.object.file.present? \
      ...
      f.input :sub_category, label: 'Sample', input_html: { class: 'sub_category'}, as: :select, collection: current_project_manager.sub_categories
      f.input :category, label: 'Species', as: :select, collection: current_project_manager.categories.uniq
      f.input :site
      f.input :location
      f.input :project
  ...
  filter :project, as: :select, multiple: true,collection: proc{current_project_manager.projects.pluck(:title, :id)}
