...
describe Job do
  ...
  describe 'ActiveModel validations' do
    ...
    describe 'Custom Validations' do
      it 'can only have offered by user as a land_manager' do
        staff1 = create(:project_manager)
        staff2 = create(:project_manager)
        job = Job.create(detail: 'Test', user: staff1, offered_by: staff2)
        expect(job.errors.messages[:offering]).to eq ["user must be a land_manager."]
      end
      it 'can only have offered to user as a project_manager' do
        customer1 = create(:land_manager)
        customer2 = create(:project_manager)
        job = Job.create(detail: 'Test', user: customer1, offered_by: customer2)
        expect(job.errors.messages[:offered]).to eq ["user must be project_manager."]
      end
    end
  end
